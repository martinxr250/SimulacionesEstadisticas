...
class ChiCuadradoPoissonRequest(BaseModel):
    rnd: List[float]  # Lista de números generados
    a: float          # Nivel de significancia
    lambd: float      # Parámetro lambda de la distribución de Poisson
    i: Optional[int] = None  # Número de intervalos (opcional)

@app.post("/tests/chi-cuadrado/poisson")
def testChiCuadradoPoissonApi(request: ChiCuadradoPoissonRequest):
    try:
        # Llamar a la función específica para la prueba de Chi-Cuadrado con Poisson
        resultado = testChiCuadradoPoisson(
            request.rnd, 
            request.a,
            request.lambd,
            request.i
