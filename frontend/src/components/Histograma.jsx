...
const Histograma = () => {
  ...
  const calcularIntervalos = () => {
    ...
    // Calcular el mínimo y máximo de los números
    let min = numeros[0]
    let max = numeros[0]
    ...
    // Ajustar el mínimo según la distribución
    // Para distribuciones que solo tienen valores positivos
    if (distribucion === "exponencial" || distribucion === "poisson") {
      // Asegurarse de que el mínimo no sea negativo para estas distribuciones
      min = Math.max(0, min)
    }

    const rango = max - min
    // Evitar división por cero si todos los números son iguales
    const anchoIntervalo = rango === 0 ? 1 : rango / intervalos
    ...
    for (let i = 0; i < numeros.length; i++) {
      ...
      if (indiceIntervalo < 0) {
        indiceIntervalo = 0
      } else if (indiceIntervalo >= intervalos) {
    ...
    for (let i = 0; i < intervalos; i++) {
      ...
      resultado.push({
        ...
        densidad: contadores[i] / (anchoIntervalo * numeros.length), // Calcular densidad
  ...
  useEffect(() => {
    if (numeros.length > 0 && plotRef.current) {
      ...
      // Determinar el rango adecuado para el histograma según la distribución
      let xmin = Math.min(...datosHistograma)
      // Para distribuciones que solo tienen valores positivos
      if (distribucion === "exponencial" || distribucion === "poisson") {
        xmin = Math.max(0, xmin)
      }
      const xmax = Math.max(...datosHistograma)
